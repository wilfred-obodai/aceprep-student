...
const AdminLogin = () => {
  ...
  const handleForgot = async (e) => {
    ...
    setForgotMsg('');
    try {
      await API.post('/auth/forgot-password', { email: forgotEmail, role: 'admin' });
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <div style={styles.header}>
          ...
          <p style={styles.tagline}>School Administration Portal</p>
          <div style={styles.badge}>🏫 School Admin Portal</div>
        ...
        {!showForgot ? (
          <>
            ...
            <form onSubmit={handleSubmit} style={styles.form}>
              <div style={styles.field}>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="school@email.com" style={styles.input} required />
              </div>
              <div style={styles.field}>
              ...
              <p style={{ ...styles.forgotLink, color: '#b8860b' }} onClick={() => setShowForgot(true)}>
                Forgot password?
              </p>
            ...
            <p style={styles.registerLink}>
              New school?{' '}
              <Link to="/admin/register" style={{ color: '#b8860b', fontWeight: 'bold' }}>Register your school</Link>
            </p>
          </>
        ) : (
          <div>
            <h3 style={{ color: '#b8860b', textAlign: 'center', marginBottom: 8 }}>🔑 Reset Password</h3>
            ...
            <p style={{ color: '#b8860b', cursor: 'pointer', textAlign: 'center', marginTop: 16, fontSize: 13 }}
              onClick={() => { setShowForgot(false); setForgotMsg(''); }}>
              ← Back to Login
            </p>
          </div>
        )}
        <p style={styles.footer}>AcePrep — Empowering Schools Across Ghana.</p>
      </div>
    </div>
  );
};
...
const styles = {
  container:   { minHeight: '100vh', background: 'linear-gradient(135deg, #7d6000 0%, #c9a800 50%, #7d6000 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 },
  ...
  backBtn:     { position: 'absolute', top: 16, left: 16, background: 'none', border: 'none', color: '#b8860b', fontSize: 14, cursor: 'pointer', fontWeight: 'bold', zIndex: 2 },
  flagStrip:   { display: 'flex', height: 6, borderRadius: 3, overflow: 'hidden', marginBottom: 16, width: '80%', margin: '0 auto 16px' },
  header:      { textAlign: 'center', marginBottom: 24 },
  logo:        { fontSize: 38, fontWeight: '900', color: '#b8860b', letterSpacing: 4, margin: '8px 0 4px' },
  ...
  btn:         { background: 'linear-gradient(135deg, #c9a800, #7d6000)', color: '#fff', border: 'none', borderRadius: 10, padding: 14, fontSize: 16, fontWeight: 'bold', cursor: 'pointer', marginTop: 4 },
