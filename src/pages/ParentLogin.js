...
const ParentLogin = () => {
  ...
  const { loginParent, loading, error } = useAuth();
  const navigate = useNavigate();

  const handleSubmit = async (e) => {
    e.preventDefault();
    const success = await loginParent(email, password);
  ...
  const handleForgot = async (e) => {
    ...
    setForgotMsg('');
    try {
      await API.post('/auth/forgot-password', { email: forgotEmail, role: 'parent' });
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <div style={styles.header}>
          ...
          <p style={styles.tagline}>Parent Monitoring Portal</p>
          <div style={styles.badge}>👨‍👩‍👧 Parent Portal</div>
        ...
        {!showForgot ? (
          <>
            ...
            <form onSubmit={handleSubmit} style={styles.form}>
              <div style={styles.field}>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="parent@email.com" style={styles.input} required />
              </div>
              <div style={styles.field}>
              ...
              <p style={{ ...styles.forgotLink, color: '#CE1126' }} onClick={() => setShowForgot(true)}>
                Forgot password?
              </p>
            ...
            <p style={styles.registerLink}>
              New parent?{' '}
              <Link to="/parent/register" style={{ color: '#CE1126', fontWeight: 'bold' }}>Register here</Link>
            </p>
          </>
        ) : (
          <div>
            <h3 style={{ color: '#CE1126', textAlign: 'center', marginBottom: 8 }}>🔑 Reset Password</h3>
            ...
            {forgotMsg === 'success' ? (
              ...
              <form onSubmit={handleForgot} style={styles.form}>
                {forgotMsg === 'error' && <div style={styles.error}>⚠️ Email not found.</div>}
                <div style={styles.field}>
                  <label style={styles.label}>Email Address</label>
            ...
            <p style={{ color: '#CE1126', cursor: 'pointer', textAlign: 'center', marginTop: 16, fontSize: 13 }}
              onClick={() => { setShowForgot(false); setForgotMsg(''); }}>
              ← Back to Login
            </p>
          </div>
        )}
        <p style={styles.footer}>AcePrep — Stay Connected to Your Child's Education.</p>
      </div>
    </div>
  );
};
...
const styles = {
  container:   { minHeight: '100vh', background: 'linear-gradient(135deg, #7a0015 0%, #CE1126 50%, #7a0015 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 },
  ...
  backBtn:     { position: 'absolute', top: 16, left: 16, background: 'none', border: 'none', color: '#CE1126', fontSize: 14, cursor: 'pointer', fontWeight: 'bold', zIndex: 2 },
  flagStrip:   { display: 'flex', height: 6, borderRadius: 3, overflow: 'hidden', marginBottom: 16, width: '80%', margin: '0 auto 16px' },
  header:      { textAlign: 'center', marginBottom: 24 },
  logo:        { fontSize: 38, fontWeight: '900', color: '#CE1126', letterSpacing: 4, margin: '8px 0 4px' },
  ...
  btn:         { background: 'linear-gradient(135deg, #CE1126, #7a0015)', color: '#fff', border: 'none', borderRadius: 10, padding: 14, fontSize: 16, fontWeight: 'bold', cursor: 'pointer', marginTop: 4 },
